...
class PpoModel(TorchModelV2, nn.Module):
    def __init__(self,
        ...
        self.policy_head = nn.Linear(hidden_dim, action_num)
    ...
    def forward(self, input_dict: dict[str, dict[str, torch.Tensor]], state=None, seq_lens=None):
        ...
        self.embed = self.encoder(input)['logits']
        logits = self.policy_head(self.embed)
        return logits, state

    def value_function(self) -> TensorType:
        return self.value_head(self.embed).squeeze(1)
