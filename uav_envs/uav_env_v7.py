...
missile = pg.transform.rotozoom(missile, 0, 0.01)
            ...
            for _ in range(self.dynamic_number):
                ...
                self.dynamic_direction.append(
                    random.randint(round(min_angular),
                                   round(max_angular)))
                self.dynamic_speed.append(random.randint(3, 5) * 0.5)
    ...
    def step(self, action: "tuple[int, int]"):
        ...
        if self.env.prevent_stiff:
            self.speed = (UAV.v_range[0]
                          + (speed_action + 1) / (UAV.nvec[0]) * (UAV.v_range[1] - UAV.v_range[0]))
        else:
            self.speed = (UAV.v_range[0]
                          + speed_action / (UAV.nvec[0] - 1) * (UAV.v_range[1] - UAV.v_range[0]))
            if self.speed == 0.:
                self.speed = 0.1
    ...
    def step(self):
        match self.mode:
            ...
            case 1:
                ...
                self.direction = (self.direction + self.angular_velocity) % 360
            case 2:
                angular_velocity = random.randint(-60, 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dynamic_obstacle_number", type=int, default=5
    )  # 动态障碍数量
