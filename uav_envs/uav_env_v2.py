...
for i in range(NUM_OBSTACLES * 2):
    MAX_OBSTACLE_LEN.append(int(np.random.uniform(10, 80)))
    MAX_OBSTACLE_WID.append(int(np.random.uniform(10, 80)))

# 随机固定障碍物
NUM_OBSTACLES_2 = 0  # disable by default
MAX_OBSTACLE_LEN_2 = []
MAX_OBSTACLE_WID_2 = []
for i in range(NUM_OBSTACLES_2 * 2):
    MAX_OBSTACLE_LEN_2.append(int(np.random.uniform(10, 30)))
    MAX_OBSTACLE_WID_2.append(int(np.random.uniform(10, 30)))
...
class UAV:
    ...
    def update_state(self):  # state加主飞机前面的随即障碍
        ...
        x_norm = self.x / self.env.dimensions[0]
        y_norm = self.y / self.env.dimensions[1]

        uav_theta_cos = math.cos(math.radians(-self.direction))
        uav_theta_cos1 = math.cos(math.radians(-self.direction + 120.))
        uav_theta_cos2 = math.cos(math.radians(-self.direction + 240.))
        uav_theta_sin = math.sin(math.radians(-self.direction))
        ...
        # goal_dy_rotated = goal_dy * uav_theta_sin - goal_dx * uav_theta_cos
        goal_dw = math.atan2(
            goal_dx_rotated,
            goal_dy_rotated,
        )

        opponent_dx = (self.x - self.other_ship.x) / self.env.dimensions[0]
        opponent_dy = (self.y - self.other_ship.y) / self.env.dimensions[1]
        opponent_dx_rotated = opponent_dx * uav_theta_cos + opponent_dy * uav_theta_sin
        opponent_dy_rotated = opponent_dy * uav_theta_cos - opponent_dx * uav_theta_sin

        nearest_obstacle_i = 0
        nearest_obstacle_j = 0
        nearest_obstacle_dist = self.env.dimensions[0] + self.env.dimensions[1]
        for i in range(0, NUM_OBSTACLES * 2, 2):
            for j in range(4):
                dist = math.hypot(
                    self.x - UAV.oxy[i] - (j % 2) * MAX_OBSTACLE_LEN[i],
                    self.y - UAV.oxy[i + 1] - (j > 1) * MAX_OBSTACLE_WID[i],
                )
                if dist < nearest_obstacle_dist:
                    nearest_obstacle_i = i
                    nearest_obstacle_j = j
                    nearest_obstacle_dist = dist
        obstacle_dx = (
                              self.x
                              - UAV.oxy[nearest_obstacle_i]
                              - (nearest_obstacle_j % 2) * MAX_OBSTACLE_LEN[nearest_obstacle_i]
                      ) / self.env.dimensions[0]
        obstacle_dy = (
                              self.y
                              - UAV.oxy[nearest_obstacle_i + 1]
                              - (nearest_obstacle_j > 1) * MAX_OBSTACLE_WID[nearest_obstacle_i]
                      ) / self.env.dimensions[1]
        obstacle_dx_rotated = obstacle_dx * uav_theta_cos + obstacle_dy * uav_theta_sin
        obstacle_dy_rotated = obstacle_dy * uav_theta_cos - obstacle_dx * uav_theta_sin
        state = np.array([
            x_norm,
            y_norm,
            uav_theta_cos,
    ...
    def update_obstacle(self):
        ...
        random_len = [
            ...
            for _ in range(NUM_OBSTACLES_2)
        ]
        random_angle = [random.randint(-90, 90) for _ in range(NUM_OBSTACLES_2)]
    ...
    def step(self, action: "tuple[int, int]"):
        ...
        reward_const = -0.1
        reward_delta_dist = self.prev_dist - dist_to_goal

        self.update_state()

        reward_goal_angle = -self.state[7]
        # 越界惩罚，结束
        reward_penalty = 0.
    ...
    def min_dist_obstacles(self):
        ...
        for i in range(0, len(UAV.oxy), 2):
            contours_list = (
                ...
                (UAV.oxy[i] + MAX_OBSTACLE_LEN[i], UAV.oxy[i + 1]),
                (
                    UAV.oxy[i] + MAX_OBSTACLE_LEN[i],
                    UAV.oxy[i + 1] + MAX_OBSTACLE_WID[i],
                ),
                (UAV.oxy[i], UAV.oxy[i + 1] + MAX_OBSTACLE_WID[i]),
            )
            contours_i = self.make_contours(contours_list)
            # contours_lists.append(contours_list)
            contours.append(contours_i)
        for a in range(len(self.obstacle)):
            if a == 0:
                b = 0
            else:
                b = a % NUM_OBSTACLES
            contours_list2 = (
                (self.obstacle[a][0], self.obstacle[a][1]),
                (self.obstacle[a][0] + MAX_OBSTACLE_LEN_2[b], self.obstacle[a][1]),
                (
                    self.obstacle[a][0] + MAX_OBSTACLE_LEN_2[b],
                    self.obstacle[a][1] + MAX_OBSTACLE_WID_2[b],
                ),
                (self.obstacle[a][0], self.obstacle[a][1] + MAX_OBSTACLE_WID_2[b]),
                (self.obstacle[a][0], self.obstacle[a][1]),
            )
            contours_a = self.make_contours(contours_list2)
            # contours_lists.append(contours_list2)
    ...
    def min_dist2(self):
        # dist1: 机身各个角点与各个障碍的距离
        # dist2: 自身中心与各个障碍的距离
        dist1, dist2 = self.min_dist_obstacles()
        try:
            if max(dist1) < 0:
                if max(dist2) < -self.length // 2:
    ...
    def draw1(self, screen):
        ...
        pg.draw.circle(
            screen, (0, 0, 255), (int(self.x), int(self.y)), self.length // 2, 1
        )  # 画圈
        pg.draw.circle(
        ...
        for i in range(len(self.obstacle)):
            ...
            pg.draw.lines(
                ...
                [
                    ...
                    (self.obstacle[i][0] + MAX_OBSTACLE_LEN_2[j], self.obstacle[i][1]),
                    (
                        self.obstacle[i][0] + MAX_OBSTACLE_LEN_2[j],
                        self.obstacle[i][1] + MAX_OBSTACLE_WID_2[j],
                    ),
                    (self.obstacle[i][0], self.obstacle[i][1] + MAX_OBSTACLE_WID_2[j]),
                    (self.obstacle[i][0], self.obstacle[i][1]),
                ],
                2,
            )
        ...
        for ii in range(0, len(UAV.oxy), 2):
            pg.draw.lines(
                ...
                [
                    ...
                    (UAV.oxy[ii] + MAX_OBSTACLE_LEN[ii], UAV.oxy[ii + 1]),
                    (
                        UAV.oxy[ii] + MAX_OBSTACLE_LEN[ii],
                        UAV.oxy[ii + 1] + MAX_OBSTACLE_WID[ii],
                    ),
                    (UAV.oxy[ii], UAV.oxy[ii + 1] + MAX_OBSTACLE_WID[ii]),
                    (UAV.oxy[ii], UAV.oxy[ii + 1]),
                ],
                2,
            )
...
class UAVEnvironment(gym.Env):
    ...
    def is_number_in_complex_ranges(self, num):
        ...
        ranges = [
            (
                (self.uav1.goal_position[0] - (self.uav1.length // 2)),
                (self.uav1.goal_position[0] + (self.uav1.length // 2)),
            ),
            (
                (self.uav1.goal_position[1] - (self.uav1.length // 2)),
                (self.uav1.goal_position[1] + (self.uav1.length // 2)),
            ),
        ]
    ...
    def reset(self, *, seed=None, options=None):
        self.uav1.goal_position = (random.randint(100, 600), random.randint(100, 600))
        uav1_init_x = np.random.uniform(
            self.dimensions[0] * 0.1, self.dimensions[0] * 0.9
        )
        uav1_init_y = np.random.uniform(
            self.dimensions[0] * 0.1, self.dimensions[0] * 0.9
        )
        uav_init_v = np.random.uniform(0, UAV.max_speed)
        uav1_init_angle = np.random.uniform(0, 360)
        # uav1_init_x = 400.
        # uav1_init_y = 400.
        # uav1_init_angle = 45.
        # goal_position = (700, 100)
        # uav1_init_angle = 135.
        # goal_position = (100, 100)
        # uav1_init_angle = 225.
        # goal_position = (100, 700)
        # uav1_init_angle = 315.
        # goal_position = (700, 700)
        # uav1_init_angle = 0.
        ...
        while self.out_of_compliance(UAV.oxy) and (
                (uav1_init_x or uav1_init_y) in UAV.oxy
        ):
            UAV.oxy = [random.randint(0, 700) for _ in range(NUM_OBSTACLES * 2)]
    ...
    def map_for_astar(self):
        ...
        for i in range(0, NUM_OBSTACLES * 2, 2):
            for j in range(UAV.oxy[i], UAV.oxy[i] + MAX_OBSTACLE_LEN[i] + 1):
                for k in range(
                        UAV.oxy[i + 1], UAV.oxy[i + 1] + MAX_OBSTACLE_WID[i] + 1
                ):
                    try:
                        grid_map[k][j] = 1
                    except:
...
if __name__ == "__main__":
    ...
    for _ in range(args.episodes):
        ...
        while not done:
            ...
            obs, reward, done, _, info = env.step([(2, 2)])
            print(f'dx: {obs[5]}')
            print(f'dy: {obs[6]}')
            print(f'dw: {obs[7]}')
            print(env.uav1.direction)
            print('-' * 50)
            if args.render:
                env.render()
