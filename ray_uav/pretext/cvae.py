...
class PretextVAE(nn.Module):
    def __init__(
        ...
        self.decoder = PretextDecoder(input_dim, embed_dim, latent_dim, hidden_dim, seq_len, num_layers, device)
...
class PretextEncoder(nn.Module):
    ...
    def predict(
            self,
            x: torch.Tensor,
            each_seq_len: Union[Any, torch.Tensor],
    ) -> torch.Tensor:
...
class PretextDecoder(nn.Module):
    def __init__(
        ...
        self.mlp_in = nn.Sequential(
            init_(nn.Linear(input_dim + latent_dim, 32)), nn.ReLU(),
            init_(nn.Linear(32, embed_dim)), nn.ReLU(),
        ).to(self.device)
        self.RNN = nn.GRU(embed_dim, hidden_dim, batch_first=True, num_layers=num_layers).to(self.device)
        for name, param in self.RNN.named_parameters():
            if 'bias' in name:
                ...
                nn.init.orthogonal_(param)
        self.output_linear = nn.Sequential(
            init_(nn.Linear(hidden_dim, hidden_dim // 2)), nn.ReLU(),
            init_(nn.Linear(hidden_dim // 2, input_dim)), nn.Sigmoid()
        ).to(self.device)
